...
func Test(t *testing.T) {
	...
	returnVal, ok := response.ReturnValue.(map[string]interface{})
	if !ok {
		t.Errorf("expected ReturnValue to be of type map[string]interface{}")
		return
	}

	data, ok := returnVal["Data"]
	if !ok {
		t.Errorf("expected ReturnValue to have key 'Data'")
		return
	}

	if data != "ArtHabitat_AHProj055_AHFlat681" {
		t.Errorf("expected ReturnValue.Data to be ArtHabitat_AHProj055_AHFlat681, got %s", data)
	}
}
